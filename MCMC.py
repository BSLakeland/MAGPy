...
class MCMC:
    def __init__(
        ...
        model_number = (
            []
        )  # This is the number of the model (depends on the order of the model list of strings)
        name_model = (
            []
        )  # This is the name of the current model (the number above does not correspond to the number in the parameter naming)
        current_mod_numb = 0
        ...
        self.hp0 = aux.initial_dist_creator(
            self.single_hp0, self.hp_err, self.numb_chains, self.hp_vary
        )
        self.modpar0 = aux.initial_dist_creator(
            self.single_modpar0, self.modpar_err, self.numb_chains, self.modpar_vary
        )
        ...
        for chain in range(self.numb_chains):
            ...
            for i, key in zip(range(len(hp_chain)), hparam_chain.keys()):
                hparam_chain[key] = Parameter.Parameter(
            ...
            for i, key in zip(range(len(modpar_chain)), modelparam_chain.keys()):
                modelparam_chain[key] = Parameter.Parameter(
            ...
            self.model_y0 = get.get_model(
                self.model_names, self.x, modelparam_chain, to_ecc=True
            )

            # Get likelihood and append to initial guess array
            self.likelyhood = lk.GPLikelihood(
                self.x,
                self.y,
                self.model_y0,
                self.y_err,
