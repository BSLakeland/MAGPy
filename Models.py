...
class Offset(Model):
    ...
    def __init__(self, x, model_par):
        ...
        assert len(self.model_par) == 1, (
            "Offset Model requires 1 parameter:" + "'offset'"
        )
        ...
        try:
            ...
            for i in range(
                20
            ):  # 20 is chosen since we don't expect a number of chosen models at the same time largern than 20 (can be changed)
                try:
...
class Keplerian(Model):
    ...
    def true_anomaly(self, E):
        ...
        """
        nu = 2.0 * np.arctan(
            np.sqrt((1.0 + self.ecc) / (1.0 - self.ecc)) * np.tan(E / 2.0)
        )
        return nu
...
class LinTrend_Model(Model):
    ...
    def __init__(self, x, model_par):
        ...
        # Check if we have the right amount of parameters
        assert len(self.model_par) == 2, (
            "Linear Trend Model requires 2 parameters:" + "'m', 'c'"
        )
            ...
            for i in range(
                ...
                try:
                    ...
                    if i == 20:
                        # if both fail and we reached the end, it means that no such parameter is given
                        raise KeyError(
                            "Linear Trend Model requires 2 parameter:" + "'m' and 'c'"
                        )
                    else:
                        continue
    ...
    def __repr__(self):
        ...
        """
        message = (
            "Linear Trend model with the parameter: \n",
            "m = {} \n".format(self.m),
            "c = {} \n".format(self.c),
        )
        print(message)
...
class UncorrNoise_Model(Model):
    ...
    def __init__(self, x, model_par):
        ...
        # Check if we have the right amount of parameters
        assert len(self.model_par) == 1, (
            "Uncorrelated Noise Model requires 1 parameters:" + "'rms'"
        )
    ...
    def __repr__(self):
        ...
        """
        message = (
            "Uncorrelated Noise model with the parameter: \n",
            "rms = {} \n".format(self.rms),
        )
        print(message)
