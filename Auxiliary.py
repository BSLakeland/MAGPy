...
def initial_dist_creator(
    params, params_err, numb_chains, vary_params=None, allow_neg=False
):
    ...
    for l in range(numb_chains - 1):
        pos = params + params_err * np.random.normal(0.0, 1.0, (1, len(params)))

        # In case we do not want to allow for negative numbers
        if not allow_neg:
            while np.min(pos) < 0:
                pos = params + params_err * np.random.uniform(
                    -1.0, 1.0, (1, len(params))
                )
