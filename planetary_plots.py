...
datafile = Path(
    "/Users/frescigno/Desktop/GP code/MAGPy final push/MR_plot_data/exoplanet.eu_catalog.csv"
)
trackfile = Path(
    "/Users/frescigno/Desktop/GP code/MAGPy final push/MR_plot_data/LZeng_tracks/Zeng2016_mrtable2.txt"
)
...
def selection(
    data, mass_limits=[0.0, 100.0], rad_limits=[0.0, 100.0], earth_units=True
):
    """
    Parameters
    ----------
    ...
    ), "mass_limits[0] must be smaller than mass_limits[1]"
    assert (
        rad_limits[0] < rad_limits[1]
    ), "rad_limits[0] must be smaller than rad_limits[1]"
    ...
    selected_data = data[
        (data["mass"] > mass_limits[0])
        & (data["mass"] < mass_limits[1])
        & (data["radius"] > rad_limits[0])
        & (data["radius"] < rad_limits[1])
        & (data["mass_error_min"] > 0.0)
        & (data["mass_error_max"] > 0.0)
        & (data["radius_error_min"] > 0.0)
        & (data["radius_error_max"] > 0.0)
        & (data["mass_error_min"] < 20.0)
        & (data["mass_error_max"] < 20.0)
    ]

    # then eliminate the stars with no characterisation or theroratical detections
    selected_data = selected_data[
        ~selected_data["detection_type"].isnull()
        & ~selected_data["mass_detection_type"].isnull()
        & ~selected_data["radius_detection_type"].isnull()
    ]
    selected_data = selected_data[
        ~selected_data["star_mass"].isnull()
        & ~selected_data["star_radius"].isnull()
...
class MR_plot:
    def __init__(self):
        ...
        self.lzeng_tracks = np.genfromtxt(
            trackfile,
            skip_header=1,
            names=[
                "Mearth",
    ...
    def get_radius_gap(self):
        self.radius_gap_x = np.arange(
            self.xlims[0] - self.xlims[0] / 2.0, self.xlims[1] + 0.1, 0.01
        )
    ...
    def setup(self):
        ...
        self.fig, self.ax1 = plt.subplots(
            1, figsize=(self.plot_size[0], self.plot_size[1])
        )
        ...
        if self.logM:
            ...
            self.ax1.xaxis.set_major_formatter(
                matplotlib.ticker.FuncFormatter(lambda x, pos: str(x))
            )
        if self.logR:
    ...
    def interpolate_line_value(
        self, mass, radius, x_pos=None, y_pos=None, default_position="top_right"
    ):
    ...
    def add_tracks(self, mass, radius, key_val=None):
        ...
        try:
            if not (key_val["x_pos"] or key_val["y_pos"]):
                ...
                x_pos, y_pos = self.interpolate_line_value(
                    mass, radius, x_pos=key_val["x_pos"]
                )
            elif key_val["y_pos"]:
                x_pos, y_pos = self.interpolate_line_value(
                    mass, radius, y_pos=key_val["y_pos"]
                )
            if key_val["rotation"]:
            ...
            print(
                key_val["label"]
                + " composition track outside the boundaries of the plot"
            )
            return
        ...
        if "overplot" in key_val:
            line = self.ax1.plot(
                mass, radius, color=color, zorder=950, ls=key_val["linestyle"]
            )
        else:
            line = self.ax1.plot(
                mass, radius, color=color, zorder=0, ls=key_val["linestyle"]
            )
        ...
        if key_val["use_box"]:
            ...
            if "fill_between" in key_val:
                self.ax1.fill_between(
                    mass, radius - 0.01, radius + 0.01, color=color, zorder=0, alpha=0.5
                )

    def plot_lzeng_tracks(self):
        for key_name in self.lzeng_plot_list:
            self.add_tracks(
                self.lzeng_tracks["Mearth"],
    ...
    def plot_radius_gap(self):
        ...
        if key_val["y_pos"]:
            ...
            x_pos, y_pos = self.interpolate_line_value(
                self.radius_gap_x, y_axis, x_pos=x_pos
            )

        if key_val["rotation"]:
            rotation = key_val["rotation"]
        else:
            rotation = self.text_slope_match_line(
                self.ax1, self.radius_gap_x, y_axis, x_pos
            )
        ...
        self.ax1.fill_between(
            self.radius_gap_x,
            self.radius_gap_y - self.radius_gap[1],
            self.radius_gap_y + self.radius_gap[1],
            color=color,
            zorder=0,
        )
    ...
    def add_my_pl(
        ...
        for ind in range(0, n_planets):
            ...
            if (
                xpos * 0.98 < self.xlims[0]
                or xpos * 1.02 > self.xlims[1]
                or ypos * 0.98 < self.ylims[0]
                or ypos > self.ylims[1]
            ):
                continue
            bbox_props = dict(
                boxstyle="square", fc="w", alpha=0.8, edgecolor="k", pad=0.2
            )
            xytext = [-7, 10]
            self.ax1.annotate(
                pl_name,
    ...
    def add_solar_system(self):
        ...
        self.ax1.plot(
            ...
            [0.949, 1.00],
            "ob",
            markersize=self.markersize + 4,
            marker="*",
            zorder=10000 + self.z_offset,
        )
        self.ax1.annotate(
            earth,
            xy=(1.0, 1.0),
