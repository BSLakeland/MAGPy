...
class Parameter_Check:
    ...
    def no_negative(self, Kernel=True):
        ...
        """
        if Kernel:
            if np.min(self.hparam) < 0.0:
                check = False
        if not Kernel:
            if np.min(self.mod_param) < 0.0:
                check = False
        return check
    ...
    def kepler_check(self):
        ...
        for name in self.model_names:
            numb_param_model = get.get_model(
                name, self.x, self.mod_param, get_number=True
            )
            if name.startswith("kepl") or name.startswith("Kepl"):

                # Period, amplitude and t0 must be positive
                if (
                    self.mod_param[current_par] < 0.0
                    or self.mod_param[current_par + 1] < 0.0
                    or self.mod_param[current_par + 4] < 0.0
                ):
                    check = False
                    return check
                ...
                if (
                    self.mod_param[current_par + 2] ** 2
                    + self.mod_param[current_par + 3] ** 2
                ) > 1.0:
                    check = False
                    return check
    ...
    def starcross_check(self, Rstar, Mstar):
        ...
        for name in self.model_names:
            ...
            if name.startswith("kepl") or name.startswith("Kepl"):
                ...
                ratio = (Rstar * Rsun / AU) / (
                    ((self.mod_param[current_par] / 365.23) ** 2) * Mstar
                ) ** (1 / 3)
