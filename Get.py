...
def get_model(model_name, time, model_par, to_ecc=False):
    ...
    for mod in model_name:
        ...
        parameters = {
            ...
            if (
                list(model_par).index(key) >= current_par_numb
                and list(model_par).index(key) < current_par_numb + model_param_number
            )
        }
        ...
        if mod.startswith("zero") or mod.startswith("Zero") or mod.startswith("no"):
            ...
            if to_ecc:
                ...
                try:
                    ...
                    try:
                        (
                            parameters["ecc_" + str(kepl_mod)].value,
                            parameters["omega_" + str(kepl_mod)].value,
                        ) = aux.to_ecc(
                            parameters["ecc_" + str(kepl_mod)].value,
                            parameters["omega_" + str(kepl_mod)].value,
                        )
                        ...
                        raise KeyError(
                            "Issue with the naming of parameters for transformation to eccentricty"
                        )
