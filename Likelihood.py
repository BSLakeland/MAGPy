...
class GPLikelihood:
    ...
    def __repr__(self):
        ...
        for i in range(len(self.hparams_values)):
            hparameters += ("{} = {} \n").format(
                self.hparams_names[i], self.hparams_values[i]
            )

        model_parameters = "Model parameters: \n"
        for i in range(len(self.mod_params_values)):
            model_parameters += ("{} = {} \n").format(
                self.mod_params_names[i], self.mod_params_values[i]
            )
    ...
    def compute_kernel(self, x1, x2):
        ...
        if (
            self.kernel_name.startswith("Periodic")
            or self.kernel_name.startswith("periodic")
            or self.kernel_name.startswith("ExpSin")
            or self.kernel_name.startswith("Expsin")
            or self.kernel_name.startswith("expsin")
        ):
        ...
        if (
            self.kernel_name.startswith("ExpSqu")
            or self.kernel_name.startswith("expsqu")
            or self.kernel_name.startswith("Expsqu")
        ):
            self.kernel = Kernels.ExpSquared(self.hparams)
        if self.kernel_name.startswith("Matern5") or self.kernel_name.startswith(
            "matern5"
        ):
    ...
    def priors(self, param_name, prior_name, prior_info):
        ...
        try:
            ...
            try:
                ...
                raise KeyError(
                    "This is not an included parameter in the list of kernel and model parameters"
                )
        ...
        if self.prior_name.startswith("Modified") or self.self.prior_name.startswith(
            "modified"
        ):
            minval = self.prior_info["minval"]
            maxval = self.prior_info["maxval"]
